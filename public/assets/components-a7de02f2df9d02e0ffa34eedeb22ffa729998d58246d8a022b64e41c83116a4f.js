  ...
  _createClass(Artist, [{
    ...
    value: function displayAdd() {
      ...
      if (imgIsChecked) {
        return React.createElement(
          "div",
          null,
          React.createElement("input", { id: this.props.songId.replace(/\W+/g, ""), type: "checkbox", className: "checkbox", name: this.props.songId.replace(/\W+/g, ""), checked: this.state.isChecked,
            onClick: function () {
        ...
        return React.createElement(
          ...
          React.createElement("label", { id: this.props.songId.replace(/\W+/g, "") + "Image", htmlFor: this.props.songId, style: styles, className: "checkbox-label" })
        );
      }
    }
  }, {
    ...
    value: function render() {
      ...
      return React.createElement(
        ...
        React.createElement(
          "div",
          { className: "nav4 card-panel height mix-color" },
          React.createElement(
            "div",
            ...
            React.createElement(
              ...
              { className: "btn individual-play-button", onClick: function () {
                  return _this4.mobilePlayButton(_this4.props.title, _this4.props.artist);
                } },
              " Play "
            )
          )
...
var PickASong = (function (_React$Component) {
  _inherits(PickASong, _React$Component);

  function PickASong(props) {
    _classCallCheck(this, PickASong);

    _get(Object.getPrototypeOf(PickASong.prototype), 'constructor', this).call(this, props);
  ...
  _createClass(PickASong, [{
    ...
    value: function attemptUpdate() {
      if (this.state.station_id != "") {
        var _that = this;
        $.ajax({
          url: "http://api.dar.fm/playlist.php?station_id=" + _that.state.station_id + "&partner_token=9388418650",
          jsonp: 'callback',
          type: 'GET',
          dataType: 'jsonp'
        }).success(function (data) {
    ...
    value: function filteredSearchResults() {
      ...
      $.ajax({
        ...
        _this3.showFilteredResults(data);
      });
        ...
        if (songs.length != 0) {
          ...
          if (this.state.searched) {
            return React.createElement(
              'div',
              { className: 'search-placeholder' },
              'Select the image of your favorite song!'
            );
          }
        }
      }
    ...
    value: function render() {
      ...
      if (this.state.results[0] != undefined) {
        ...
        searchResultCards = this.state.results.length ? this.state.results[0].songmatch.map(function (Sartist) {
          return React.createElement(Artist, { title: Sartist.title, songIndex: "result" + self.state.results[0].songmatch.indexOf(Sartist), songs: _this5.state.songs, getSongs: _this5.getSongs, artist: Sartist.artist, key: 'artist-' + Sartist.artist + Sartist.title, mixtapeId: self.state.mixtape_id, current_user: self.props.current_user, changeStationId: _this5.changeStationId, songId: Sartist.artist.replace(/\W+/g, "") + Sartist.title.replace(/\W+/g, ""), check: _this5.check, unCheck: _this5.unCheck });
        }) : [];
      }
      var songs = self.state.songs.map(function (song) {
        ...
        return React.createElement(
          ...
          React.createElement(SelectedArtist, { songs: _this5.state.songs, key: 'mixtapeSong-' + song.artist, songIndex: "favorite" + self.state.songs.indexOf(song), title: song.song_name, artist: song.artist_name, songId: "selected-" + song.song_name.replace(/\W+/g, "") + song.artist_name.replace(/\W+/g, ""), onChange: _this5.changeHandler, getSongs: _this5.getSongs, changeStationId: _this5.changeStationId, doneButton: _this5.doneButton, removeDoneButton: _this5.removeDoneButton }),
          React.createElement(
            'p',
      ...
      return React.createElement(
        ...
        React.createElement(
          'a',
          { href: '/', id: 'done-button', className: 'edit-profile-button hidden-mobile' },
          'Done'
        ),
        ...
        React.createElement('input', { id: 'search', className: 'large-search word-spacing', type: 'text', ref: 'searchText', autofocus: 'true', placeholder: 'Song or Artist' }),
        React.createElement(
          'button',
...
var PlayMySongs = (function (_React$Component) {
  ...
  _createClass(PlayMySongs, [{
    ...
    key: 'noSongsAdded',
    value: function noSongsAdded() {
      if (this.state.songs.length == 0) {
        return React.createElement(
          'p',
          { id: 'no-songs-notification', className: 'notification-spacing' },
          ' It looks like this user doesn\'t have any songs added. '
        );
      } else {
        this.playMode();
      }
    ...
    value: function playSingleSong(song) {
      ...
      if (/Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(navigator.userAgent)) {
        window.open('http://onrad.io/?msi={"artist":"' + song.artist_name + '","title":"' + song.song_name + '","profileurl":"' + this.props.profile_image_url + '","profilename":"' + this.props.username + '"}');
    ...
    value: function playMultipleSongs() {
      ...
      $.ajax({
        ...
        if (data[0].success) {
          if (/Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(navigator.userAgent)) {
            //window.open("http://onrad.io/" + data[0].result[0].artist + "." + data[0].result[0].title);
            window.open('http://onrad.io/?msi={"artist":"' + data[0].result[0].artist + '","title":"' + data[0].result[0].title + '","profileurl":"' + _this2.props.profile_image_url + '","profilename":"' + _this2.props.username + '"}');
            that.albumCover(data[0].songmatch[0].title, data[0].songmatch[0].artist);
          } else {
            var player = document.getElementById("player");
            player.src = "http://api.dar.fm/player_api.php?station_id=" + data[0].result[0].station_id + "&custom_style=radioslice&partner_token=9388418650";
    ...
    value: function albumCover(title, artist) {
      ...
      $.ajax({
        ...
        document.getElementById("show-artist").innerHTML = artist;
    ...
    value: function render() {
      ...
      if (this.state.results[0] != undefined) {
        searchResultCards = this.state.results.length ? this.state.results[0].songmatch.map(function (Sartist) {
          return React.createElement(Artist, { title: Sartist.title, songIndex: "result" + self.state.results[0].songmatch.indexOf(Sartist), songs: _this5.state.songs, getSongs: _this5.getSongs, artist: Sartist.artist, key: 'artist-' + (i += 1), rplay: self.playSong, mixtapeId: self.state.mixtape_id, current_user: self.props.current_user, changeStationId: _this5.changeStationId, songId: Sartist.artist.replace(/\s/g, "") + Sartist.title.replace(/\s/g, "") });
        }) : [];
      }
      var songs = self.state.songs.map(function (song) {
        // let key = `mixtapeSong-${song.song_id}`;
        return React.createElement(
          'div',
          { className: 'inline-container' },
          React.createElement(SelectedArtist, { songs: _this5.state.songs, key: 'mixtapeSong-' + song.song_id, songIndex: "favorite" + self.state.songs.indexOf(song), title: song.song_name, artist: song.artist_name, songId: "selected" + song.song_id, onChange: _this5.changeHandler, getSongs: _this5.getSongs, changeStationId: _this5.changeStationId }),
          React.createElement(
            'p',
  ...
  _createClass(SelectedArtist, [{
    ...
    value: function displayAdd() {
      ...
      return React.createElement(
        ...
        React.createElement("label", { id: this.props.songId.replace(/\W+/g, "") + "Image", htmlFor: this.props.songId, style: styles, className: "mysong-label image-checked" })
      );
    }
  }, {
    key: "render",
...
var ViewPlayer = (function (_React$Component) {
  _inherits(ViewPlayer, _React$Component);

  function ViewPlayer(props) {
    _classCallCheck(this, ViewPlayer);

    _get(Object.getPrototypeOf(ViewPlayer.prototype), "constructor", this).call(this, props);
    this.state = { stationUrl: "" };
  }
  ...
  _createClass(ViewPlayer, [{
    ...
    value: function mobilePlayer() {
      if (/Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(navigator.userAgent)) {
        ...
        return React.createElement("iframe", { className: "", id: "player", src: this.stationUrl });
      }
    }
  }, {
    key: "render",
